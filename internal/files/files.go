...
func EBRepoFileSetup(sdk *sdk.Sdk, githubRepoDetails setup.GithubRepoDetails, rdsBool bool, rdsDetails awsrds.RDSDetails) (string, error) {
	githubDownloadLink, err := getDownloadLink(githubRepoDetails)
	if err != nil {
		return "", err
	}

	err = download(sdk, fmt.Sprintf("%s.zip", githubRepoDetails.Repo), githubDownloadLink)
	if err != nil {
		return "", err
	}
...
func download(sdk *sdk.Sdk, filepath string, url string) error {
	logger.LogSlack(sdk, "🔄 Downloading repository files...")
	...
	logger.LogSlack(sdk, "✅ Download complete.")
	return err
}
